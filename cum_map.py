...
def render_interactive_dose_map(dose_map, title, vmax=None, roi_used=None, roi_list=None, peak_xy=None):
    ...
    if roi_used is not None:
        ...
        fig.add_annotation(
            x=roi_used[0], y=roi_used[1],
            text=f"ROI {roi_used[2]-roi_used[0]}×{roi_used[3]-roi_used[1]} px",
            showarrow=False, xanchor="left", yanchor="bottom",
            font=dict(color="white"), bgcolor="rgba(0,0,0,0.45)"
        )
    if roi_list:
        for i, roi in enumerate(roi_list, start=1):
            fig.add_shape(
                ...
                line=dict(color="cyan", width=1), fillcolor="rgba(0,255,255,0.0)"
            )
            fig.add_annotation(
                x=roi[0], y=roi[1], text=f"R{i}", showarrow=False,
                xanchor="left", yanchor="bottom", font=dict(color="cyan"),
                bgcolor="rgba(0,0,0,0.35)"
            )
    if peak_xy is not None:
        fig.add_scatter(
            x=[peak_xy[0]], y=[peak_xy[1]],
            mode="markers+text",
            text=["Peak"], textposition="top center",
            marker=dict(size=10, symbol="x", color="white"),
            name="Peak", hovertemplate="Peak X: %{x}<br>Peak Y: %{y}<extra></extra>"
        )
    return fig
...
def roi_stats(cumulative_dose, roi=None):
    ...
    peak_val = float(np.max(stats_map)) if stats_map.size else 0.0
    peak_idx_local = np.unravel_index(np.argmax(stats_map), stats_map.shape) if stats_map.size else (0, 0)
...
def build_csv_bytes(session_records, overall_stats, multi_roi_stats):
    ...
    if multi_roi_stats:
        lines.append("")
        lines.append("roi_label,roi,peak_cumulative_dose_gy,peak_x,peak_y,mean_dose_gy,median_dose_gy,std_dose_gy,min_nonzero_dose_gy,p95_dose_gy,dose_sum,nonzero_pixels,pixels_ge_2gy,pixels_ge_5gy,pixels_ge_10gy")
...
uploaded_files = st.file_uploader("Upload OpenREM PNG images", type=["png"], accept_multiple_files=True)
...
if uploaded_files:
    ...
    size_mode = st.selectbox(
        ...
        ["Strict (all reconstructed maps must match exactly)", "Crop to smallest common size", "Pad to largest common size"],
        index=1,
    )
    ...
    if "cumulative_dose" in st.session_state:
        ...
        if add_roi_clicked and roi_used is not None:
            st.session_state.roi_list.append({"label": roi_label, "roi": roi_used})
